...
```bash
chmod +x Stayline-1.1.4-x86_64.AppImage
./Stayline-1.1.4-x86_64.AppImage
```
...
If an older Stayline is already in the tray, the new AppImage will only focus that old window. Choose **Quit** from the tray, then start `Stayline-1.1.4-x86_64.AppImage`.
